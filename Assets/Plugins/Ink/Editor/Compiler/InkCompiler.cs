...
namespace Ink.UnityIntegration {
	...
	public static class InkCompiler {
		...
		private static void OnPlayModeChange ( PlayModeStateChange newState ) {
			if(newState == PlayModeStateChange.EnteredEditMode && InkLibrary.Instance.pendingCompilationStack.Count > 0) {
			...
			if(newState == PlayModeStateChange.ExitingEditMode && compiling)
